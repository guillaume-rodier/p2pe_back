...
const createTables = async () => {
  await pool.query('CREATE TABLE IF NOT EXISTS users (id Serial PRIMARY KEY,email VARCHAR UNIQUE,password VARCHAR,gender VARCHAR,address VARCHAR,phone_number VARCHAR UNIQUE,first_name VARCHAR,last_name VARCHAR,role VARCHAR,company_name VARCHAR,company_description VARCHAR,number_employee INTEGER,siret VARCHAR UNIQUE,state VARCHAR,profession VARCHAR,image_screen TEXT)',
    (error, results) => {
      ...
      pool.query(
        "CREATE TABLE IF NOT EXISTS proposed_services (id SERIAL, name VARCHAR, description VARCHAR, location VARCHAR, price FLOAT, creation_date timestamp, state VARCHAR, rate FLOAT, option VARCHAR,id_pro SERIAL, PRIMARY KEY (id), FOREIGN KEY (id_pro) REFERENCES users(id))",
        (error, results) => {
          if (error) {
          ...
          pool.query(
            ...
            (error, results) => {
              if (error) {
                console.log(
                  "Cannot create tables requested_services : " + error.message,
                  results
                );
                return;
              }
