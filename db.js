...
const createTables = async () => {
 ...
 await pool.query('CREATE TABLE IF NOT EXISTS users (id Serial PRIMARY KEY,email VARCHAR UNIQUE,password VARCHAR,gender VARCHAR,address VARCHAR,phone_number VARCHAR UNIQUE,first_name VARCHAR,last_name VARCHAR,role VARCHAR,company_name VARCHAR,company_description VARCHAR,number_employee INTEGER,siret VARCHAR UNIQUE,state VARCHAR,profession VARCHAR)', (error, results) => {
    ...
    pool.query('CREATE TABLE IF NOT EXISTS proposed_services (id SERIAL, name VARCHAR, description VARCHAR, location VARCHAR, price FLOAT, creation_date TIME, state VARCHAR, rate FLOAT, option VARCHAR, id_pro SERIAL, PRIMARY KEY (id), FOREIGN KEY (id_pro) REFERENCES users(id))', (error, results) => {
      if (error) {
